...
1. In the command line, and enter the following command to run the program:
    ...
    **C#**
    ...
    > **Tip**: If a compilation error occurs because the code targets .NET version 8.0 but .NET 9.0 is installed in the cloud shell, edit the **test-model.csproj** file in the code folder and update the **TargetFramework** setting to **9.0**.
